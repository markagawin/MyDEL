...
import { computeCreditCardBalance, isCreditCardPayment, isCreditPurchase } from '../creditCard';
...
export default function CreditCardSummaryModal({ visible, transactions, categoryMap, onClose }: Props) {
  const theme = useTheme();
  const styles = useMemo(() => createStyles(theme), [theme]);

  const creditCardTransactions = useMemo(
    () => transactions.filter((t) => isCreditPurchase(t) || isCreditCardPayment(t)),
    [transactions]
  );

  const balance = useMemo(() => computeCreditCardBalance(transactions), [transactions]);
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <SafeAreaView style={styles.safe} edges={['top']}>
        <View style={styles.header}>
          <TouchableOpacity accessibilityLabel="Close credit card summary" onPress={onClose} style={styles.closeButton}>
        ...
        <View style={styles.totalCard}>
          <Text style={styles.totalLabel}>Credit Card Owed</Text>
          <Text style={styles.totalValue}>{formatPeso(balance)}</Text>
        </View>

        {sections.length === 0 ? (
          <View style={styles.emptyState}>
            <Text style={styles.emptyText}>No credit card purchases or payments logged yet.</Text>
          </View>
        ) : (
          <SectionList
            sections={sections}
            ...
            renderItem={({ item }) => {
              const isPayment = isCreditCardPayment(item);
              const meta = isPayment ? null : categoryMap[item.category] ?? UNKNOWN_CATEGORY;
              return (
                <View style={styles.row}>
                  <View style={styles.rowMiddle}>
                    <Text style={styles.rowLabel}>
                      {isPayment ? '💳 Payment' : `${meta!.icon} ${meta!.label}`}
                    </Text>
                  ...
                  <Text style={[styles.rowAmount, isPayment && styles.rowAmountPayment]}>
                    {isPayment ? '− ' : '+ '}
...
const createStyles = (theme: AppTheme) =>
  StyleSheet.create({
    ...
    rowAmountPayment: { color: theme.success },
  });
