...
export default function BorrowEntryModal({
  ...
  const goToAmount = (action: BorrowAction) => {
  ...
  const handleSubmit = () => {
    ...
    onSubmit({ borrowerId, borrowAction, amount: amountValue, note, date: entryDate });
  };
      ...
      <Modal visible={visible} animationType="fade" transparent onRequestClose={onClose}>
        <Pressable style={styles.backdrop} onPress={onClose}>
          <Pressable style={styles.sheet} onPress={() => {}} onStartShouldSetResponder={() => true}>
            ...
            {step === 2 && (
              <>
                <Text style={styles.title}>Borrow, or pay back?</Text>
                {borrowerName ? <Text style={styles.subtitle}>{borrowerName}</Text> : null}
                <View style={styles.bigButtonRow}>
                  <TouchableOpacity style={styles.bigButton} onPress={() => goToAmount('borrow')}>
                    <Text style={styles.bigButtonIcon}>📥</Text>
                    <Text style={styles.bigButtonText}>Borrow</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.bigButton} onPress={() => goToAmount('paid_back')}>
                    <Text style={styles.bigButtonIcon}>💸</Text>
                    <Text style={styles.bigButtonText}>Paid Back</Text>
                  </TouchableOpacity>
                </View>
              </>
            )}
            ...
            {step === 3 && (
              <>
                <Text style={styles.title}>
                  {borrowAction === 'paid_back' ? 'How much did you pay back?' : 'How much did you borrow?'}
                </Text>
                {borrowerName ? <Text style={styles.subtitle}>{borrowerName}</Text> : null}
