...
export default function QuickLogScreen() {
  ...
  // cash has actually left yet, only the eventual "Pay Credit Card" entry does. Borrowing money
  // is the mirror of that: it's real cash in hand right away, so it's subtracted (increasing
  // what's left) the moment it's borrowed, and paying it back adds it back. This is what
  ...
  const handleLendingSubmit = (data: LendingEntrySubmission) => {
    const borrowerName = borrowers.find((b) => b.id === data.borrowerId)?.name ?? 'them';
    const now = new Date();
    const timestamp = new Date(
      data.date.getFullYear(),
    ...
    const newId = addTransaction({
      amount: data.amount,
      category: LENDING_CATEGORY_KEY,
      note: data.note,
      timestamp,
      borrowerId: data.borrowerId,
      lendingAction: data.lendingAction,
    });
